...
%% Task 1.3+
R_vals = [0.1, 0.01, 0.001, 10, 100];
C_hat_1_vals = [[0.5; 1], [1.3; 5], [-1; 10], [50; 100]];
k_count = [50, 100, 500];

for T = k_count
    for C_hat_1 = C_hat_1_vals
        for R = R_vals
            y = noisy_meas_seq(T, C, covar);
            C_hat = estimate_seq(T, y)
            C_plot = ones([T 2]) .* C.';
            err_sq = (C_hat - C_plot) .^ 2;
...
function C_hat = estimate_seq(k_count, y_seq)
