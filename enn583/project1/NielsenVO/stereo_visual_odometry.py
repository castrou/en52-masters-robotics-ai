...
class VisualOdometry():
    def __init__(self, data):
        self.K_l, self.P_l, self.K_r, self.P_r = self._load_calib(data)
        self.gt_poses = self._load_poses(data)
        self.images_l, self.images_r = self._load_images(data)
    ...
    def _load_calib(data):
        ...
        """
        P_l = data.calib.P_rect_20
        K_l = data.calib.K_cam2
        P_r = data.calib.P_rect_30
        K_r = data.calib.K_cam3
        return K_l, P_l, K_r, P_r

    @staticmethod
    def _load_poses(data):
        """
        Loads the GT poses
        ...
        """
        poses = [pose for pose in np.array([oxts.T_w_imu for oxts in data.oxts])]
        return poses
    ...
    def _load_images(data):
        ...
        right = []
        for l, r in data.rgb:
            left.append(cv2.cvtColor(np.array(l), cv2.COLOR_RGB2GRAY))
            right.append(cv2.cvtColor(np.array(r), cv2.COLOR_RGB2GRAY))
        return left, right
